...
export default function FilaOrdens({ user }) {
    ...
    useEffect(() => {
        ...
        const updateTimer = () => {
            ...
            const targetTime = new Date(selectedOS.data_hora).getTime();
    ...
    async function fetchNotas(os_id) {
        try {
            const token = localStorage.getItem('token');
            const config = { headers: { 'Authorization': `Bearer ${token}` } };
            const res = await api.get(`/ordens/${os_id}/notas`, config);
            setNotas(res.data);
        } catch (error) {
    ...
    async function handleStatusChange(newStatus) {
        ...
        if (window.confirm(`Tem certeza que deseja alterar o status para "${newStatus}"?`)) {
            try {
                const token = localStorage.getItem('token');
                const config = { headers: { 'Authorization': `Bearer ${token}` } };
                await api.put(`/ordens/${selectedOS.id}/status`, { status: newStatus }, config);
    ...
    async function handleRescheduleSubmit() {
        ...
        try {
            const token = localStorage.getItem('token');
            const config = { headers: { 'Authorization': `Bearer ${token}` } };
            await api.put(`/ordens/${selectedOS.id}/reagendar`, { novaDataHora: rescheduleDateTime }, config);
    ...
    async function handleExcluirOS(osId) {
        if (window.confirm('Tem certeza que deseja excluir esta OS? O lançamento financeiro correspondente (se houver) não será alterado.')) {
            try {
                const token = localStorage.getItem('token');
                const config = { headers: { 'Authorization': `Bearer ${token}` } };
                await api.delete(`/ordens/${osId}`, config);
    ...
    async function handleAddNota() {
        ...
        try {
            const token = localStorage.getItem('token');
            const config = { headers: { 'Authorization': `Bearer ${token}` } };
            await api.post(`/ordens/${selectedOS.id}/notas`, { autor: user.nome || "Usuário", nota: novaNota }, config);
            setNovaNota('');
    ...
    const ordensFiltradas = useMemo(() => {
        ...
        if (filterData.data_resolucao) filtered = filtered.filter(os => new Date(os.data_resolucao).toISOString().split('T')[0] === filterData.data_resolucao);
        return filtered;
                        ...
                        <div className="os-details-layout">
                            <div className="os-details-column">
                                <div className="os-info-grid">
                                    ...
                                    <p><strong>Data/Hora:</strong> {new Date(selectedOS.data_hora).toLocaleString('pt-BR')}</p>
                                    ...
                                    {notas.length > 0 ? (
                                        notas.map(n => (
                                            <div key={n.id} className="chat-bubble">
                                                ...
                                                <span>{new Date(n.data_hora).toLocaleString('pt-BR')}</span>
                                            </div>
                                        ))
                                    ) : (
