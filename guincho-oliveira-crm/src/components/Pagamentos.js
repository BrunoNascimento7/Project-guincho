...
export default function Pagamentos() {
    ...
    async function fetchData() {
        try {
            ...
            const [pagamentosRes, motoristasRes, transacoesRes] = await Promise.all([
                api.get('/financeiro?tipo=Despesa', config),
            ...
            setResumo({ receita, despesa, saldo });
            setChartData([
                { name: 'Receita', valor: receita },
                { name: 'Despesa', valor: despesa },
    ...
    const filteredPagamentos = useMemo(() => {
        return pagamentos.filter(p => {
            ...
            const itemDescricao = p.descricao.toLowerCase();
            const filterDate = filters.data;
            const itemDate = p.data.split('T')[0];
    ...
    async function handleSubmit(e) {
        ...
        try {
            const token = localStorage.getItem('token');
            const payload = { ...formData, tipo: 'Despesa', categoria_id: 2 };
                                ...
                                {filteredPagamentos.length > 0 ? filteredPagamentos.map(p => (
                                    <li key={p.id}>
                                        <div className="list-item-info">
                                            ...
                                            <span className="pagamento-meta">{new Date(p.data).toLocaleDateString('pt-BR')} | {motoristas.find(m => m.id === p.motorista_id)?.nome || 'N/A'}</span>
                                        </div>
